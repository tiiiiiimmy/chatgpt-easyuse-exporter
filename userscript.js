...
// @author       Alex Mercer, Hanashiro, WenDavid; Enhanced by LeYangZi, assisted by Claude and Codex
...
(function () {
    ...
    async function ensureAccessToken() {
        ...
        alert('无法获取 Access Token。请刷新页面或打开任意一个对话后再试。\n\nFailed to obtain Access Token. Please refresh the page or open any conversation and try again.');
        return null;
    }
    ...
    // --- 会话解析（按时间排序的完整映射） Conversation parsing (full mapping sorted by time) ---
...
<body>
    <div class="container">
        ...
        <div class="footer">由 ChatGPT Universal Exporter Enhanced v8.3.1 导出 / Exported by ChatGPT Universal Exporter Enhanced v8.3.1</div>
    </div>
</body>
</html>`;
        return html;
    ...
    async function startExportProcess(mode, workspaceId, formats) {
        ...
        btn.disabled = true;
        if (!await ensureAccessToken()) { btn.disabled = false; btn.textContent = '导出会话 / Export Conversations'; return; }
        try {
            const zip = new JSZip();
            btn.textContent = '📂 获取项目外对话… / Fetching conversations outside projects…';
            const orphanIds = await collectIds(btn, workspaceId, null);
            for (let i = 0; i < orphanIds.length; i++) {
                btn.textContent = `📥 根目录 / Root (${i + 1}/${orphanIds.length})`;
            ...
            btn.textContent = '🔍 获取项目列表… / Fetching project list…';
            const projects = await getProjects(workspaceId);
            for (const project of projects) {
                const projectFolder = zip.folder(sanitizeFilename(project.title));
                btn.textContent = `📂 项目 / Project: ${project.title}`;
            ...
            btn.textContent = '📦 生成 ZIP 文件… / Generating ZIP file…';
            ...
            btn.textContent = '✅ 完成 / Done';
        } catch (e) {
            console.error("导出过程中发生严重错误", e);
            alert(`导出失败: ${e.message}。详情请查看控制台（F12 -> Console）。\n\nExport failed: ${e.message}. See the browser console (F12 -> Console) for details.`);
            btn.textContent = '⚠️ 错误 / Error';
        } finally {
            setTimeout(() => { btn.disabled = false; btn.textContent = '导出会话 / Export Conversations'; }, 3000);
        }
    }
    ...
    async function collectIds(btn, workspaceId, gizmoId) {
        ...
        if (gizmoId) {
            ...
            for (const is_archived of [false, true]) {
                ...
                do {
                    btn.textContent = `📂 项目外对话 / Non-project conversations (${is_archived ? '已归档 / Archived' : '活动 / Active'} p${++page})`;
    ...
    function showExportDialog() {
        ...
        dialog.innerHTML = `
            <h2 style="margin:0 0 12px 0;font-size:18px;">导出会话 / Export Conversations</h2>
            ...
            <div style="margin:10px 0;">
                <label><input type="radio" name="mode" value="personal" checked> 个人空间 / Personal</label>
                <label style="margin-left:12px;"><input type="radio" name="mode" value="team"> 团队空间 / Team</label>
            </div>
            <div id="team-area" style="display:none;">
                <div style="font-size:12px;color:#555;margin-bottom:6px;">自动检测到的 Workspace IDs（如有）/ Detected Workspace IDs (if any):</div>
                <div id="detected"></div>
                <input type="text" id="team-id" placeholder="或在此粘贴 Team Workspace ID (ws-...) / Or paste Team Workspace ID here (ws-...)" style="width:100%;padding:8px;border:1px solid #ddd;border-radius:6px;">
            </div>
            <div style="margin-top:14px;display:flex;justify-content:flex-end;gap:10px;">
                <button id="dlg-cancel" style="padding:8px 12px;border:1px solid #ccc;border-radius:8px;background:#fff;cursor:pointer;">取消 / Cancel</button>
                <button id="dlg-start" style="padding:8px 12px;border:none;border-radius:8px;background:#10a37f;color:#fff;cursor:pointer;font-weight:bold;">开始导出 / Start Export</button>
            </div>
        `;
        ...
        dialog.querySelector('#dlg-start').onclick = async () => {
            ...
            if (!formats.json && !formats.markdown && !formats.html) { alert('请至少选择一种导出格式！\n\nPlease select at least one export format!'); return; }
            ...
            if (mode === 'team') {
                ...
                if (!workspaceId) { alert('请选择或输入一个有效的 Team Workspace ID！\n\nPlease select or enter a valid Team Workspace ID!'); return; }
            }
    ...
    function addBtn() {
        ...
        const b = document.createElement('button'); b.id = 'gpt-rescue-btn'; b.textContent = '导出会话 / Export Conversations';
